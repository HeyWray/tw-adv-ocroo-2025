...
4. In order to complete the project, we need to install OpenCV. Fill in the following:
  ...
  > https://github.com/HeyWray/tw-adv-ocroo-2025)
...
6. Have the dependencies in the `pyproject.toml` changed? If so, how?
  >opencv-python has been added
  >
7. Why did we use `uv add` over `uv pip`?
  >uv pip adds the package to the .venv. uv add, does that and adds it to the dependecy.
  >
8. The `numpy` library is required for OpenCV. Should you add an explicit requirement for it? Why/Why not?
  >No. While numpy is needed it is not pertinent to the importance of the project. Would we add in the dependency we need electricity and a computer?
  >
...
> 
> Does it have external dependencies - Tesseract.
> 
  ...
  "videos": [
    {
      ...
      "_links": {
        "self": "/video/demo",
        "frame_example": "/video/demo/frame/1.0"
      }
    }
  ]
}
